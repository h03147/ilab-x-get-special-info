...
export function export_json_to_excel({
    ...
    if(filename === 'ilab-x筛选信息表')
    {
        chemicaltableheader.forEach(function (val) {
            dataInfo[val].s = tableTitleFont; // 给表头列加样式
            // dataInfo[val].s = {border: borderAll}; // 给表头列加边框
        });
    }else
    {
        chemicaltableheader2.forEach(function (val) {
            dataInfo[val].s = tableTitleFont; // 给表头列加样式
            // dataInfo[val].s = {border: borderAll}; // 给表头列加边框
        });
    }
